...
class NUMPYIEKF:
    ...
    def set_param_attr(self):
        ...
        attr_list = [a for a in dir(self.filter_parameters) if not a.startswith('__') and not callable(getattr(self.filter_parameters, a))]
    ...
    def run(self, t: list[float], u: list[Any], measurements_covs: list[Any], v_mes: list[Any], p_mes: list[Any], N: int | None, ang0: list[Any]):
        """
        Run the IEKF filter for a given time series of measurements.
        ...
        for i in range(1, N):
            ...
            Rot[i], v[i], p[i], b_omega[i], b_acc[i], Rot_c_i[i], t_c_i[i], P = \
                self.update(Rot[i], v[i], p[i], b_omega[i], b_acc[i], Rot_c_i[i], t_c_i[i], P, u[i], i, measurements_covs[i])
    ...
    def rot_from_2_vectors(v1, v2):
        ...
        Rot = NUMPYIEKF.Id3 + NUMPYIEKF.skew(v) + NUMPYIEKF.skew(v).dot(NUMPYIEKF.skew(v)) * (1 - cosang) / (sinang ** 2)
    ...
    def sen3exp(xi):
        ...
        if np.abs(angle) < 1e-8:
            ...
            s = np.sin(angle)
            c = np.cos(angle)
            J = (s / angle) * NUMPYIEKF.Id3 + (1 - s / angle) * np.outer(axis, axis) + ((1 - c) / angle) * skew_axis
    ...
    def so3left_jacobian(phi):
        ...
        return (s / angle) * NUMPYIEKF.Id3 + (1 - s / angle) * np.outer(axis, axis) + ((1 - c) / angle) * skew_axis
    ...
    def set_learned_covariance(self, torch_iekf):
        """
        Set the learned covariance matrices from a TORCHIEKF instance.
        This method extracts the learned covariance matrices from the TORCHIEKF instance
        and updates the corresponding attributes in the NUMPYIEKF instance.
        """
        torch_iekf.set_Q()
