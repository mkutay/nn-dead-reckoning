...
def IEKF_plot(args, dataset):
    for i in range(0, len(dataset.datasets)):
        ...
        for j in range(Rot.shape[0]):
            roll, pitch, yaw = to_rpy(torch.from_numpy(Rot[j]))
            ang[j, 0] = roll.numpy()
            ang[j, 0] = pitch.numpy()
            ang[j, 0] = yaw.numpy()
            # unwrap
            Rot_gt[j] = TORCHIEKF.from_rpy(torch.Tensor([ang_gt[j, 0]]), torch.Tensor([ang_gt[j, 1]]), torch.Tensor([ang_gt[j, 2]]))
            roll, pitch, yaw = to_rpy(Rot_gt[j])
        ...
        v_r_gt = (Rot_gt.transpose(-1, -2).matmul(torch.from_numpy(v_gt).float().unsqueeze(-1)).squeeze()).numpy()
        ...
        axs1[2].set(xlabel='time (s)', ylabel='$\mathbf{R}_n^T \mathbf{v}_n$ (m/s)', title="Velocity in body frame")
        axs2[0].set(xlabel='time (s)', ylabel=r'$\phi_n, \theta_n, \psi_n$ (rad)', title="Orientation")
        axs2[1].set(xlabel='time (s)', ylabel=r'$\mathbf{b}_{n}^{\mathbf{\omega}}$ (rad/s)', title="Bias gyro")
        axs2[2].set(xlabel='time (s)', ylabel=r'$\mathbf{b}_{n}^{\mathbf{a}}$ (m/$\mathrm{s}^2$)', title="Bias accelerometer")
        ...
        axs5[0].set(xlabel='time (s)', ylabel=r' $\mathrm{cov}(\mathbf{y}_{n})$ (log scale)', title="Covariance on the zero lateral and vertical velocity measurements (log scale)")
        axs5[1].set(xlabel='time (s)', ylabel=r'Normalized gyro measurements', title="Normalized gyro measurements")
        axs5[2].set(xlabel='time (s)', ylabel=r'Normalized accelerometer measurements', title="Normalized accelerometer measurements")
        axs6[0].set(xlabel='time (s)', ylabel=r'$\omega^x_n, \omega^y_n, \omega^z_n$ (rad/s)', title="Gyrometer")
        axs6[1].set(xlabel='time (s)', ylabel=r'$a^x_n, a^y_n, a^z_n$ (m/$\mathrm{s}^2$)', title="Accelerometer")
        axs7[0].set(xlabel='time (s)', ylabel=r'$|| \mathbf{p}_{n}-\hat{\mathbf{p}}_{n} ||$ (m)', title="Mean Absolute Trajectory Error (MATE) and Root Mean Square Error (RMSE)")
        axs7[1].set(xlabel='time (s)', ylabel=r'$\Sigma_{i=0}^{n} || \mathbf{p}_{i}-\hat{\mathbf{p}}_{i} ||$ (m)', title="Cumulative Absolute Trajectory Error (CATE)")
        axs7[2].set(xlabel='time (s)', ylabel=r' $\mathbf{\xi}_{n}^{\mathbf{p}}$', title="$SE(3)$ error on position")

        for ax in chain(axs1, axs2, axs5, axs6, axs7):
            ax.grid()
        ax3.grid()
        ax4.grid()
        axs1[0].legend(['$p_n^x$', '$p_n^y$', '$p_n^z$', '$\hat{p}_n^x$', '$\hat{p}_n^y$', '$\hat{p}_n^z$'])
        axs1[1].legend(['$v_n^x$', '$v_n^y$', '$v_n^z$', '$\hat{v}_n^x$', '$\hat{v}_n^y$', '$\hat{v}_n^z$'])
        axs1[2].legend(['$v_n^x$', '$v_n^y$', '$v_n^z$', '$\hat{v}_n^x$', '$\hat{v}_n^y$', '$\hat{v}_n^z$'])
        axs2[0].legend([r'$\phi_n^x$', r'$\theta_n^y$', r'$\psi_n^z$', r'$\hat{\phi}_n^x$', r'$\hat{\theta}_n^y$', r'$\hat{\psi}_n^z$'])
        axs2[1].legend(['$b_n^x$', '$b_n^y$', '$b_n^z$', '$\hat{b}_n^x$', '$\hat{b}_n^y$', '$\hat{b}_n^z$'])
        axs2[2].legend(['$b_n^x$', '$b_n^y$', '$b_n^z$', '$\hat{b}_n^x$', '$\hat{b}_n^y$', '$\hat{b}_n^z$'])
        ...
        figs_name = ["position_velocity", "orientation_bias", "position_xy", "position_xy_aligned", "measurements_covs", "imu", "errors", "errors2"]
